...
public class Menu {
    ...
    private JTextField txtInsertFlightOrigin;
    private JTextField txtInsertFlightDestination;
    private JTextField txtInsertFlightDate;
    private JTextField txtInsertFlightPrice;
    ...
    public Menu() {
        ...
        insertFlightButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String origin = txtInsertFlightOrigin.getText();
                String destination = txtInsertFlightDestination.getText();
                String d_flight = txtInsertFlightDate.getText();
                String price = txtInsertFlightPrice.getText();
                //comprobaciones
                if (origin.isEmpty() || destination.isEmpty() || d_flight.isEmpty() || price.isEmpty()) {
                    JOptionPane.showMessageDialog(null, "Please fill all the fields");
                } else {
                    //introducimos datos en la DB
                    try {
                        String insertDataFlight = "INSERT INTO flight(origin, destination, d_flight, price) VALUES ('" + origin + "','" + destination + "','" + d_flight + "','" + price + "')";
                        PreparedStatement ps = (PreparedStatement) cn.prepareStatement(insertDataFlight);
                        ps.executeUpdate();
                        cleanForms();
                        JOptionPane.showMessageDialog(null, "Flight inserted successfully");

                    } catch (Exception exception) {
                        JOptionPane.showMessageDialog(null, "An error ocurred" + exception);
                    }
                }
            }
        });
